...
public class OpenGlHelper
{
    ...
    public static void glShaderSource(int shaderIn, ByteBuffer string)
    {
        if (arbShaders)
        {
            ...
            string.get(bytes);
            string.rewind();
            ARBShaderObjects.glShaderSourceARB(shaderIn, new String(bytes, java.nio.charset.StandardCharsets.UTF_8));
        }
        else
        {
            byte[] bytes = new byte[string.remaining()];
            string.get(bytes);
            string.rewind();
            GL20.glShaderSource(shaderIn, new String(bytes, java.nio.charset.StandardCharsets.UTF_8));
        }
    }
