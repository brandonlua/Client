...
public class GuiVideoSettings extends GuiScreenOF
{
    ...
    public void initGui()
    {
        ...
        this.buttonList.add(new GuiOptionButton(231, i1, l, Lang.get("of.options.shader")));
    ...
    private void actionPerformed(GuiButton p_actionPerformed_1_, int p_actionPerformed_2_)
    {
        if (p_actionPerformed_1_.enabled)
        {
            ...
            if (p_actionPerformed_1_.id == 231)
            {
                ...
                {
                    Config.showGuiMessage(Lang.get("of.message.shader.aa1"), Lang.get("of.message.shader.aa2"));
                    return;
                }

                if (Config.isAnisotropicFiltering())
                {
                    Config.showGuiMessage(Lang.get("of.message.shader.af1"), Lang.get("of.message.shader.af2"));
                    return;
                }

                if (Config.isFastRender())
                {
                    Config.showGuiMessage(Lang.get("of.message.shader.fr1"), Lang.get("of.message.shader.fr2"));
                    return;
                }

                if (Config.getGameSettings().anaglyph)
                {
                    Config.showGuiMessage(Lang.get("of.message.shader.an1"), Lang.get("of.message.shader.an2"));
                    return;
                }
