...
/**
 * Startup intro: black screen, then the Rania icon fades in/out (~1s), then
 * "Welcome to RaniaClient" fades in/out (~1s), then the main menu appears with a
 * fade-in (handled by {@link GuiMainMenu}'s own fade overlay).
 */
public class GuiSplash extends GuiScreen {

    private static final ResourceLocation ICON = new ResourceLocation("textures/rania/icon.png");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (t < ICON_END) {
            ...
            FontProcess.getScaledFont("sans", 2.5f)
                    .drawCenteredString("Welcome to RaniaClient", this.width / 2, this.height / 2 - 8, (a << 24) | 0xFFFFFF);
