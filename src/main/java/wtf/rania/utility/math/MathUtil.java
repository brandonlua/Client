package wtf.rania.utility.math;
...
public class MathUtil {
    ...
    public static Vec3 interpolate(Vec3 end, Vec3 start, float multiple) {
        return new Vec3(
                interpolate(end.xCoord, start.xCoord, (double) multiple),
                interpolate(end.yCoord, start.yCoord, (double) multiple),
                interpolate(end.zCoord, start.zCoord, (double) multiple));
    }
    ...
    public static double interpolate(double old, double now) {
        ...
        try {
            ...
            return interpolate(old, now, timer.renderPartialTicks);
        } catch (Exception e) {
            return old;
        }
    }
    ...
    public static float calculateGaussianValue(float x, float sigma) {
        ...
        double output = 1.0 / Math.sqrt(2.0 * PI_VAL * (sigma * sigma));
