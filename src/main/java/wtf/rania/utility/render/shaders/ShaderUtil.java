package wtf.rania.utility.render.shaders;
...
public class ShaderUtil implements InstanceAccess {
    ...
    private final String mainmenu = """
        ...
        void mainImage(out vec4 fragColor, in vec2 fragCoord) {
            ...
            float t = TIME * 0.2;
            vec2 flow = uv * 2.0;
            flow += vec2(fbm(flow + t), fbm(flow - t)) * 0.6;
            float liquid = fbm(flow * 1.5 + t * 0.5);
            liquid = smoothstep(0.1, 0.9, liquid);
            vec3 col = mix(THEME_COLOR_DARK, THEME_COLOR, liquid);
            float r = length(uv);
